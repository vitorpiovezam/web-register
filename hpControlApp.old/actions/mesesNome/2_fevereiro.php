...
$result = mysqli_query($connection,"select replace(responsavel , ' ','-') from Fevereiro");
...
while ($property = mysqli_fetch_field($result)) {
    echo '<td>' . 'Responsavel' . '</td>';  //get field name for header
...
while ($row = mysqli_fetch_array($result)) {
    ...
    foreach ($all_property as $item) {
      $link = "'popup.php?cat_id=$row[$item]&mes=Fevereiro',";
      $title = "'resultado',";
      $width = "'width=500px,";
      $height = "height=400px'";
